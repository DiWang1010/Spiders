...
class downloader(object):
    def __init__(self):
        ...
        self.target=r'https://www.9siwa.com/forum.php?mod=viewthread&tid=25116&extra=page%3D7'
        # self.check_content()
        self.read_content()
    ...
    def read_content(self):
        ...
        self.texts = bf.find_all('div',class_='t_fsz')
        temp=BeautifulSoup(str(self.texts))
        self.texts=temp.find_all('img')
        id=1
        for each in self.texts:
            # print(each.get('file'))
            try:
                img = requests.get(each.get('file'))
            except:
